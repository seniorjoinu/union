...
export class CandidParser extends EmbeddedActionsParser {
        ...
        this.OR([
            {
                ALT: () => {
                    let name: string;
                    this.OR1([
                        { ALT: () => { name = this.CONSUME(Nat).image } },
                        { ALT: () => { name = this.SUBRULE(this.name) } },
                    ]);
    ...
    public comment = this.RULE('comment', () => {
        ...
        this.OR([
            { ALT: () => {
                comment = this.CONSUME(SLComment).image.replace(/(\/\/)|(\n)/, '').trim()
            } },
            { ALT: () => {
                comment = this.CONSUME(MLComment).image.replace(/(\/\*)|(\*\/)/, '').trim()
            } },
        ]);
