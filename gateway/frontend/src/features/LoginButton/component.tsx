...
export const LoginButton = mobxReactLite.observer(({ mnemonic, backUrl, children, onLogin, height = 16, ...props }: LoginButtonProps) => {
  ...
  const { authClient, login, logout } = useAuth();
	...
	return (
		<Container>
			<CroppedString variant='p1' onClick={() => navigator.clipboard.writeText(principal)}>{principal}</CroppedString>
			<Button
				{...props}
				id='login'
				variant='text'
