...
pub struct SharesMoveRepository {
    ...
    pub records_by_timestamp_index: HashMap<(GroupId, Principal), SortedByTimestamp<SharesMoveEntryId>>,
}
...
impl SharesMoveRepository {
    ...
    pub fn entry_of_at(&self, group_id: GroupId, of: Principal, timestamp: u64) -> Option<SharesMoveEntry> {
        let sorted = self.records_by_timestamp_index.get(&(group_id, of))?;
        let entries = sorted.most_actual_by(&timestamp)?;
        assert_eq!(entries.len(), 1);
        
        let id = entries.get(&0).unwrap();
        
        self.records.get(id).cloned()
    }

    fn add_to_timestamp_index(&mut self, group_id: GroupId, principal: Principal, timestamp: u64, id: SharesMoveEntryId) {
        self.records_by_timestamp_index.entry((group_id, principal)).or_default().push(timestamp, id);
    }
