...
describe('groups', () => {
    ...
    it('open groups work fine', async () => {
        ...
        const {balance: user1Balance1} = await user1.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
        const {balance: user2Balance1} = await user2.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
        ...
        const {balance: user1Balance2} = await user1.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
        const {balance: user2Balance2} = await user2.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
        ...
        const {balance: user1Balance3} = await user1.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
        const {balance: user2Balance3} = await user2.walletPersonal.actor.get_my_group_shares_balance({
            group_id: groupId
        });
